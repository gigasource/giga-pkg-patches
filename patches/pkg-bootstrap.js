...
function wrapperFindPath({VIRTUAL_FILESYSTEM, STORE_BLOB, STORE_CONTENT, STORE_LINKS}) {
  const ENOENT = 2
  ...
  function findNativeAddonForInternalModuleStat (path_) {
    var path = findNativeAddonSyncUnderRequire(path_);
    if (!path) return -ENOENT;
    return internalModuleStat(makeLong(path));
  }
  ...
  function tryFile(requestPath, isMain) {
    ...
    if (process.env.NODE_PRESERVE_SYMLINKS && !isMain) {
      return rc === 0 && path.resolve(requestPath);
    }
    return rc === 0 && toRealPath(requestPath);
  }

  let internalModuleReadJSON, internalModuleReadFile;

  internalModuleReadFile = internalModuleReadJSON = function (long) {
    // from node comments:
    // Used to speed up module loading. Returns the contents of the file as
    // a string or undefined when the file cannot be opened. The speedup
    // comes from not creating Error objects on failure.
  ...
  const internalModuleStat = function (long) {
    // from node comments:
    // Used to speed up module loading. Returns 0 if the path refers to
    // a file, 1 when it's a directory or < 0 on error (usually -ENOENT).
    // The speedup comes from not creating thousands of Stat and Error objects.
  ...
  function stat(filename) {
    ...
    if (cache !== null && !FLAG_ENABLE_PROJECT) {
...
(function () {
  ...
  ancestor._findPath = Module._findPath
  Module._findPath = wrapperFindPath({VIRTUAL_FILESYSTEM, STORE_BLOB, STORE_CONTENT, STORE_LINKS});
  ...
  Module._resolveFilename = function () {
    var filename;
    var flagWasOn = false;
    /*if (arguments[0].includes('snapshot')) {
      return arguments[0];
    }*/
    try {
